...
#[cfg(test)]
mod tests {
    use _common::ApiError;
    use super::*;
    static TEST_PART1_STR: &str = include_str!("test_part1.txt");
    static TEST_PART2_STR: &str = include_str!("test_part2.txt");

    #[test]
    fn test_part01() -> Result<(), ApiError> {
        let mut aoc_day = {{class_name}}::new(TEST_PART1_STR).ok_or(ApiError::Error)?;
        Ok(assert_eq!(aoc_day.process_part_01(), 999))
    }
    #[test]
    fn test_part02() -> Result<(), ApiError> {
        let mut aoc_day = {{class_name}}::new(TEST_PART2_STR).ok_or(ApiError::Error)?;
        Ok(assert_eq!(aoc_day.process_part_02(), 999))
    }
    #[test]
    fn test_part01_final() -> Result<(), ApiError> {
        let mut aoc_day = {{class_name}}::new(INPUT_STR).ok_or(ApiError::Error)?;
    ...
    #[test]
    fn test_part02_final() -> Result<(), ApiError> {
        let mut aoc_day = {{class_name}}::new(INPUT_STR).ok_or(ApiError::Error)?;
